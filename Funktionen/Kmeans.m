...
% Auskommentiert von Jochen Lohmiller am 16.04.2014
...
% distNames = {'sqeuclidean','cityblock','cosine','correlation','hamming'}; % GEH von Jochen Lohmiller am 20.07.2011 hinzugefügt.
distNames = {'sqeuclidean','cityblock','cosine','correlation','hamming','geh','vortisch_formel', 'pillat_formel', 'euclidean_mit_nan'}; % GEH von Jochen Lohmiller am 20.07.2011 hinzugefügt.
        ...
        function converged = onlineUpdate()
            ...
            while iter < maxit
                ...
                switch distance
                    ...
                    % von Jochen Lohmiller am 16.04.2014 hinzugefügt !!!! 
                    case 'geh' 
                        Del=GEH(X,C);
                    case 'vortisch_formel'
                        Del=Vortisch_formel(X,C);
                    case 'pillat_formel'
                        Del=Pillat_formel(X,C);
                    case 'euclidean_mit_nan'
                        Del=Euclidean_mit_NaN(X,C);
                    % Ende eingefügt.   
                ...
                m(oidx) = m(oidx) - 1;
                switch distance
                    case {'sqeuclidean', 'geh', 'vortisch_formel', 'pillat_formel', 'euclidean_mit_nan'} % von Jochen Lohmiller am 16.04.2014 erweitert: DAVOR: case 'sqeuclidean'
...
switch dist
    ...
    case 'geh' 
        D=GEH(X,C);
    case 'vortisch_formel'
        D=Vortisch_formel(X,C);
    case 'pillat_formel'
        D=Pillat_formel(X,C);
    case 'euclidean_mit_nan'
        D=Euclidean_mit_NaN(X,C);
        % ENDE einfügen.
...
for i = 1:num
    ...
    if any(members)
        ...
        switch dist
            ...
            % von Jochen Lohmiller am 16.04.2014 hinzugefügt !!!!
            case {'geh', 'vortisch_formel', 'pillat_formel', 'euclidean_mit_nan'}
                XMembers = X(members,:);
                Summen = zeros(1, size(XMembers,2)); % vordimensionieren
                for cnt_Z = 1 : size(XMembers,2),
                    Summen(cnt_Z) = sum(XMembers(~isnan(XMembers(:, cnt_Z)), cnt_Z));
                end
                centroids(i,:) = Summen / counts(i);
            % ENDE einfügen.
